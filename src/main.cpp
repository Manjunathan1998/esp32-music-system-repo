...
void setupEncoderFocusGroup()
{
  ...
  lv_group_add_obj(focus_group, objects.equalizer_button);
  ...
  lv_obj_add_flag(objects.equalizer_button, LV_OBJ_FLAG_SCROLL_ON_FOCUS | LV_OBJ_FLAG_CLICKABLE);
...
void updateBatteryCharge()
{
  ...
  lv_label_set_text(objects.battery_skeleton, buffer);
        ...
        for (int i = 0; i < 4; ++i)
        {
          if (focused == menu_buttons[i])
          {
            ...
            case 0:
              ...
              break;
            case 1:
              cmd = CMD_SWITCH_TO_SCR_AWS;
              Serial.println("CMD_SWITCH_TO_SCR_AWS");
              break;
            case 2:
